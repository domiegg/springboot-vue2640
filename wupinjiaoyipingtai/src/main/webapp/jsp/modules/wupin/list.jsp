...
<body>
    <div class="modal fade" id="plusOrReduceWupinKucunNumberModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <%-- 模态框标题--%>
                <div class="modal-header">
                    <h5 class="modal-title" id="plusOrReduceWupinKucunNumberTitle">增加或减少库存</h5>
                ...
                <div class="modal-body">
                    数量:<input type="number" id="plusOrReduceWupinKucunNumber" style="width: 300px;" class="form-control form-control-sm"
                    ...
                    <input type="hidden" id="wupinId">
                    <input type="hidden" id="hideflag">
                    <input type="hidden" id="thisWupinKucunNumber">
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" onclick="submitPlusOrReduceWupinKucunNumber()" class="btn btn-primary">提交</button>
                </div>
            </div>
        </div>
    </div>
...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">物品列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                            ...
                            <div class="table-responsive mb-3">
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        现价
                                        <div class="layui-input-inline">
                                            <input type="text" id="wupinNewMoneyStartSearch" style="width: 100px;" class="form-control form-control-sm" onchange="wupinNewMoneyChickValue(this)"
                                                   placeholder="开始" aria-controls="tableId">
                                        </div>
                                    </div>
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        <div class="layui-input-inline">
                                            <input type="text" id="wupinNewMoneyEndSearch" style="width: 100px;" class="form-control form-control-sm" onchange="wupinNewMoneyChickValue(this)"
    ...
    <script>
        ...
        function setDataRow(item, number) {
            ...
            //物品库存
            var wupinKucunNumberCell = document.createElement('td');
            wupinKucunNumberCell.innerHTML = item.wupinKucunNumber;
            row.appendChild(wupinKucunNumberCell);
            ...
            //点击次数
            var wupinClicknumCell = document.createElement('td');
            wupinClicknumCell.innerHTML = item.wupinClicknum;
            row.appendChild(wupinClicknumCell);
            ...
            var plusAttr = "plusWupinKucunNumber(" + item.id+','+item.wupinKucunNumber+")";
            ...
            var reduceAttr = "reduceWupinKucunNumber(" + item.id+','+item.wupinKucunNumber+')';
        ...
        function shangxia(id,shangxiaTypes) {
            ...
            httpJson("wupin/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList()
                }else{
    ...
    //查询当前表搜索条件所有列表
        ...
        //增加库存
        function plusWupinKucunNumber(thisId,wupinKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceWupinKucunNumberTitle").val("增加库存");
            $("#thisWupinKucunNumber").val(wupinKucunNumber);
            $("#hideflag").val("+");
            $("#wupinId").val(thisId);
            id=thisId;
            $('#plusOrReduceWupinKucunNumberModal').modal('show');
        }

        //减少库存
        function reduceWupinKucunNumber(thisId,wupinKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceWupinKucunNumberTitle").val("减少库存");
            $("#thisWupinKucunNumber").val(wupinKucunNumber);
            $("#hideflag").val("-");
            $("#wupinId").val(thisId);
            id=thisId;
        ...
        $('#plusOrReduceWupinKucunNumberModal').on('show.bs.modal', function () {
            //清理表格内容,防止重复回显
            $("#plusOrReduceWupinKucunNumber").val(0);
            if(id ==null){
                alert("获取当前列id错误,请联系管理员处理");
                $('#plusOrReduceWupinKucunNumberModal').modal('hide');
                $("#plusOrReduceWupinKucunNumberTitle").val("");
                $("#thisWupinKucunNumber").val("");
                $("#hideflag").val("");
                $("#wupinId").val("");
                getDataList();
                return;
        ...
        //模态框关闭的时候会执行这个里面的东西
        $('#plusOrReduceWupinKucunNumberModal').on('hide.bs.modal', function () {
            id=null;
            $("#plusOrReduceWupinKucunNumber").val(0);
            $("#plusOrReduceWupinKucunNumberTitle").val("");
            $("#thisWupinKucunNumber").val("");
        ...
        function submitPlusOrReduceWupinKucunNumber() {
            var id = $("#wupinId").val();//当前数据id
            var hideflag = $("#hideflag").val();//是加还是减
            var thisWupinKucunNumber = $("#thisWupinKucunNumber").val();//原先的数量
            var plusOrReduceWupinKucunNumber = $("#plusOrReduceWupinKucunNumber").val();//增加或者减少的数量
            var sumNumber=0;
            var msg;
            if(hideflag == "+"){
                sumNumber =parseInt(thisWupinKucunNumber) + parseInt(plusOrReduceWupinKucunNumber);
                msg = "增加库存成功";
            }else if(hideflag == "-"){
                sumNumber =parseInt(thisWupinKucunNumber) - parseInt(plusOrReduceWupinKucunNumber);
                msg = "减少库存成功";
            ...
            let data = {"id":id,"wupinKucunNumber":sumNumber};
            httpJson("wupin/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList();
