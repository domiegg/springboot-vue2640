...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的物品订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="modal-dialog" style="z-index: 3000">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <textarea id="wupinCommentbackContent" style="width: 500px;height: 300px"></textarea>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" @click="submitWupinCommentback()" class="btn btn-theme">评价</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部物品订单</li>
                    <li v-for="item in wupinOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.wupinName}}</td>
                            <td style="width: 250px;">
                                <img :src="item.wupinPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.wupinValue}}</td>
                            <td style="width: 60px;">{{item.wupinKucunNumber}}</td>
                            <td style="width: 100px;color: red">{{item.wupinOldMoney}}</td>
                            <td style="width: 100px;color: red">{{item.wupinNewMoney}}</td>
                            <td style="width: 60px;">{{item.wupinClicknum}}</td>
                            <td style="width: 80px;">{{item.shangxiaValue}}</td>
                            <td style="width: 150px;">{{item.wupinOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.wupinOrderTruePrice}}</td>
                            <td style="width: 150px;">{{item.wupinOrderCourierName}}</td>
                            <td style="width: 150px;">{{item.wupinOrderCourierNumber}}</td>
                            <td style="width: 80px;">{{item.wupinOrderValue}}</td>
                            <td style="width: 80px;">{{item.wupinOrderPaymentValue}}</td>
                            ...
                            <td style="width: 100px;">
                                ...
                                <button v-if="item.wupinOrderTypes==3" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 退款
                                </button>


                                <button v-if="item.wupinOrderTypes==4" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.wupinOrderTypes==5" @click="commentback(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        // 当前表字典表数据
            wupinOrderTypesList : [],
            wupinOrderPaymentTypesList : [],
            wupinOrderTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            //列表查询
            ,pageList(wupinOrderTypes) {
                this.wupinOrderTypes = wupinOrderTypes;
                // 获取列表数据
                layui.http.request('wupinOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    wupinOrderTypes: (vue.wupinOrderTypes == 0)?null:vue.wupinOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('wupinOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    wupinOrderTypes: (this.wupinOrderTypes == 0)?null:this.wupinOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`wupinOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`wupinOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`wupinOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,submitWupinCommentback(){
                var wupinCommentbackContent = $("#wupinCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("wupinOrder/commentback?id="+id+"&commentbackText="+wupinCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function wupinOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wupin_order_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.wupinOrderTypesList = res.data.list;
                }
            });
        }
        // 支付类型的查询
        function wupinOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wupin_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.wupinOrderPaymentTypesList = res.data.list;
                }
            });
        }
        ...
        // 获取列表数据
        http.request('wupinOrder/page', 'get',{
            page: 1,
            limit: limit,
            wupinOrderTypes: (vue.wupinOrderTypes == 0)?null:vue.wupinOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('wupinOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            wupinOrderType: (vue.wupinOrderTypes == 0)?null:vue.wupinOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
