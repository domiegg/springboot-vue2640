    ...
    <div class="page-order-comfirm">
        <div class="section-con">
			...
			<div class="style-list pay">
				<ul class="clearfix">
					<li :class="{'selected':wupinOrderPaymentTypes==1}" @click="wupinOrderPaymentTypes = 1">线上支付</li>
					<li :class="{'selected':wupinOrderPaymentTypes==2}" @click="wupinOrderPaymentTypes = 2">线下支付</li>
				</ul>
			</div>
	
			<div v-if="wupinOrderPaymentTypes == 1">
            ...
            <div class="orderBox">
                <div class="orderC-table">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td class="text-left">
                                <div class="order-gw clearfix">
                                    <div class="order-img"><img :src="item.wupinPhoto"/></div>
                                    <div class="order-sum">
                                        <h2>{{item.wupinName}}</h2>
                                    </div>
                                </div>
                            </td>
                            <td>{{item.wupinNewMoney}} RMB</td>
                            <td>
                                {{item.buyNumber}}
                            </td>
                            <td >
                                <span class="priceD">￥{{(item.wupinNewMoney*item.buyNumber).toFixed(2)}}</span>
                            </td>
                        </tr>
                        </tbody>
                    </table>

                    <div class="orderC-total" v-if="this.wupinOrderPaymentTypes == 1">
                        <div class="order-t">
                            <dl class="clearfix">
                    ...
                    <div class="orderC-total" v-if="this.wupinOrderPaymentTypes == 2">
                        <div class="order-t">
                            <dl class="clearfix">
                                <dd class="dl_ddL">总&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;价：</dd>
                                <dd class="dl_ddR">0元</dd>
                            </dl>
                        </div>
                        <!--<div class="order-sum">
                            <dl class="clearfix">
		...
		<script>
			var vue = new Vue({
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.wupinNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						...
						let data={
                            wupins:localStorage.getItem('wupins'),
                            yonghuId: localStorage.getItem('userid'),
                            wupinOrderPaymentTypes: vue.wupinOrderPaymentTypes,
						}
                        if(vue.wupinOrderPaymentTypes == 1){
                            ...
                            data["addressId"] = vue.addressList[index].id;
                        }
                        // 获取商品详情信息
                        layui.http.request(`wupinOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../wupinOrder/list.jsp';
							}
                       		 window.location.href='../wupinOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取物品购买的清单列表
				var wupins = localStorage.getItem('wupins');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(wupins);
