        ...
        <div class="cartBox">
            ...
            <div class="order_content">
                <ul class="order_lists" v-for="(item,index) in dataList" v-bind:key="index">
                    ...
                    <li class="list_con">
                        <div class="list_img"><img :src="item.wupinPhoto"></div>
                        <div class="list_text">{{item.wupinName}}</div>
                    </li>
                    <li class="list_price">
                        <p class="price">{{item.wupinNewMoney}} RMB</p>
                    </li>
                    ...
                    <li class="list_sum">
                        <p class="sum_price">{{(item.wupinNewMoney*item.buyNumber).toFixed(2)}} RMB</p>
                    </li>
                    <li class="list_op">
    ...
    var vue = new Vue({
        ...
        methods: {
			...
			,addCartNum(index) {
				...
				layui.http.request(`wupin/info/${item.wupinId}`, 'get', {}, function(res) {
					// 库存限制
					if (res.data.wupinKucunNumber && res.data.wupinKucunNumber > 0 && res.data.wupinKucunNumber  <= item.buyNumber) {
						layui.layer.msg(`物品库存不足`, {
							time: 2000,
							icon: 5
						});
						return
			...
			// 购买
			buyClick() {
				localStorage.setItem('wupins', JSON.stringify(this.dataList));
				window.location.href = '../wupinOrder/confirm.jsp'
			}
        }
    });
